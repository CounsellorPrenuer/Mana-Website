import { cn } from "@/lib/utils";

/** Header wordmark only — navy letters + magenta i, no tagline or A flourishes. */
export default function MentoriaWordmark({
  className,
  dark = false,
}: {
  className?: string;
  dark?: boolean;
}) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="248 78 555 112"
      fill="none"
      role="img"
      aria-label="Mentoria"
      className={cn("block h-full w-full", dark && "brightness-0 invert", className)}
      preserveAspectRatio="xMinYMid meet"
    >
      <path
        d="M251.08 112.65h17.59L288.18 144l19.5-31.37h17.6v74.19h-16.22V138.4l-20.88 31.7h-.43l-20.67-31.38v48.12h-16zm89.99 0h56v14.52h-39.78v15h35v14.52h-35v15.58h40.28v14.52h-56.5zm66.58 0h15.06l34.76 45.68v-45.68h16.12v74.19H459.7l-35.93-47.16v47.16h-16.12zM505 127.7h-22.63v-15h61.48v15h-22.58v59.14H505zm45 22.3v-.21c0-21.1 16.64-38.38 39.54-38.38s39.32 17.07 39.32 38.16v.22c0 21.09-16.64 38.37-39.54 38.37S550 171.05 550 150zm61.8 0v-.21c0-12.73-9.33-23.32-22.48-23.32S567 136.81 567 149.53v.22c0 12.71 9.33 23.31 22.48 23.31s22.28-10.38 22.28-23.06zm27.93-37.35h33.91c9.44 0 16.75 2.65 21.63 7.52 4.13 4.14 6.36 10 6.36 17v.21c0 12-6.47 19.51-15.9 23l18.12 26.5h-19.08l-15.9-23.74h-12.82v23.74h-16.32zm32.85 36c8 0 12.51-4.24 12.51-10.49V138c0-7-4.87-10.6-12.82-10.6h-16.22v21.3zm98.27-36.53h15.06l31.79 74.72h-17.06l-6.78-16.64h-31.38l-6.78 16.64h-16.65z"
        fill={dark ? "#ffffff" : "#00005a"}
      />
      <path
        d="M721.37 101.82h-5.51c-.72 0-1-.21-1.11-1l-2.64-18.68c-.17-1.14.13-1.44 1.51-1.49A55.23 55.23 0 0 0 724 79.38a18.54 18.54 0 0 0 6.23-2.5c3.57-2.39 5.06-5.94 4.14-9.93-.83-3.64-3.66-6-7.9-6.58-7.56-1.08-14.06 1.27-19.77 6.14a14.76 14.76 0 0 0-1 .9c-1.52 1.36-1.51 1.35-2.84-.12l-9.45-10.34c-.59-.63-.75-1 0-1.66 8-8.08 17.46-12.62 29-12.7 6.21 0 12.26.64 17.9 3.38 8 3.9 12.39 10.38 13.39 19.17a26.45 26.45 0 0 1-2 14.66c-2.82 5.9-7.61 9.58-13.6 11.86a49.12 49.12 0 0 1-7.67 2.24c-.79.16-1.22.39-1.33 1.3-.22 1.81-.68 3.6-.93 5.41-.13 1-.55 1.29-1.53 1.24-1.84-.09-3.56-.03-5.27-.03zm-.46 8.58h8.64c1 0 1.24.31 1.14 1.21l-1 10.19q-1 10-2.14 20l-1.94 18.27c-.07.75-.38.85-1 .85-2.34 0-4.68-.08-7 0-1 0-1.37-.27-1.46-1.25l-1.59-15.17-2.66-25-.76-8.09c0-.81.21-1.07 1-1.06l8.77.05zM712 178v-7.63c0-.84.27-1 1.07-1q7.83 0 15.65 0c.86 0 1 .3 1 1.08v15.39c0 .9-.28 1.1-1.13 1.09h-15.52c-.91 0-1.11-.29-1.1-1.14L712 178z"
        fill={dark ? "#f4c40e" : "#e60c56"}
      />
    </svg>
  );
}
